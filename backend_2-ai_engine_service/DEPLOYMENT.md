# SkillSync AI Engine Service - Deployment Guide
...
services:
  ...
  watchtower:
    ...
    command: --interval 60 --include-restart skillsync-ai-engine
    restart: unless-stopped
...
Create `/etc/systemd/system/skillsync-ai-engine.service`:

```ini
[Unit]
Description=SkillSync AI Engine Service
...
```bash
sudo cp skillsync-ai-engine.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now skillsync-ai-engine
```
...
docker inspect --format='{{.State.Health.Status}}' skillsync-ai-engine
...
docker run --rm -v /var/run/docker.sock:/var/run/docker.sock \
  containrrr/watchtower --run-once skillsync-ai-engine
```
...
# Kill and restart
docker kill $(docker ps -q --filter ancestor=skillsync-ai-engine)
docker rm skillsync-ai-engine
docker run -d --name skillsync-ai-engine -p 9000:9000 ...
```
