# SkillSync - Full Deployment Guide

> Complete deployment instructions for all SkillSync backend services

---

## Overview

SkillSync consists of 3 backend microservices:
...
|----------|---------|-------------|
| DATABASE_URL | postgresql://...@db:5432/skillsync | PostgreSQL connection |
...
docker run -d -p 9000:9000 skillsync-ai-engine
```
...
# Or manually
docker build -t skillsync-ai-companion .
docker run -d -p 9001:9001 skillsync-ai-companion
```
...
abc123         skillsync-core-api         Up 2 minutes    0.0.0.0:8000->8000/tcp
def456         skillsync-ai-engine        Up 2 minutes    0.0.0.0:9000->9000/tcp
ghi789         skillsync-ai-companion     Up 2 minutes    0.0.0.0:9001->9001/tcp
jkl012         skillsync-db               Up 3 minutes    0.0.0.0:5432->5432/tcp
mno345         skillsync-redis            Up 3 minutes    0.0.0.0:6379->6379/tcp
```
...
SkillSync requires initial course data to generate recommendations. A CSV of NSQF courses must be placed at `backend/data/nsqf_courses.csv` (can be copied from `backend_2-ai_engine_service/data/nsqf_courses.csv`) and seeded using:
```bash
cd backend
...
### View Logs
```bash
docker logs skillsync-core-api
docker logs skillsync-ai-engine
docker logs skillsync-ai-companion
```
