# SkillSync Core API - Deployment Guide
...
set DATABASE_URL=postgresql://postgres:postgres@localhost:5432/skillsync
...
|---------|-----------|------|-------------|
| API | skillsync-core-api | 8000 | FastAPI application |
| PostgreSQL | skillsync-db | 5432 | Database |
| Redis | skillsync-redis | 6379 | Cache & Queue |
| Worker | skillsync-worker | - | Celery worker |
...
|----------|---------|-------------|
| `DATABASE_URL` | postgresql://postgres:postgres@db:5432/skillsync | PostgreSQL connection |
...
services:
  watchtower:
    ...
    command: --interval 60 --include-restart skillsync-core-api
...
def456         skillsync-db          Up 3 minutes    0.0.0.0:5432->5432/tcp
ghi789         skillsync-redis       Up 3 minutes    0.0.0.0:6379->6379/tcp
jkl012         skillsync-worker      Up 2 minutes
```
...
# Worker logs
docker logs skillsync-worker

# Database logs
docker logs skillsync-db
```
...
docker logs skillsync-redis

# Test connection
docker exec -it skillsync-redis redis-cli ping
```
...
docker pull ghcr.io/<username>/skillsync/b1-core:latest
```
